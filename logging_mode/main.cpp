...
int main(int argc, char** argv) {
    ...
    parser.program_name("logging-mode")
        .program_description("Shows the correct way of using custom argument value types")
        .default_arguments({ ap::default_argument::o_help });

    parser.add_optional_argument<logging_mode>("logging-mode", "l")
        .default_value(logging_mode::quiet)
        .implicit_value(logging_mode::normal)
        .nargs(1);
